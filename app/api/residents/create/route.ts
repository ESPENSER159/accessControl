...
export async function POST(request: Request) {
    ...
    try {
        // Validate if exist
        const search: any = await conn.query('SELECT name, address FROM condominiums WHERE name = ? AND address = ?', [name, address])
        ...
        // Create
        const create: any = await conn.query('INSERT INTO condominiums (name, address, creation_date) VALUES (?, ?, ?)', [name, address, creationDate])
